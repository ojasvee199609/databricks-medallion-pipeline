"""Bronze ingestion for product source data.

Reads the oldest pending products*.csv from to_process (pattern-matched),
writes bronze_products, then archives the file to processed with its original
...
from bronze_config import (  # noqa: E402
    PRODUCTS_SOURCE_FILENAME_PATTERN,
    PRODUCTS_SOURCE_SCHEMA,
    IngestResult,
...
def ingest_products(spark: SparkSession) -> IngestResult:
    """Ingest raw product CSV data into the bronze_products Delta table.
    ...
    try:
        ...
        raw_df = read_csv_with_schema(spark, source_path, PRODUCTS_SOURCE_SCHEMA)
...
def main() -> int:
    """Run product Bronze ingestion as a standalone script.
